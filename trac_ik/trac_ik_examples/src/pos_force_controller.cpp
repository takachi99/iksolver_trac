...
class pos_force_controller{
  ...
  private:
    ...
    // vector<double> pose_pid_gain{0.003,0.0,0.0003};//{P,I,D}
    vector<double> pose_pid_gain{0.06*7.0,0.0,0.001*7.0};//{P,I,D}
    ...
    // vector<double> force_pid_gain{0.0001*0.1,0.0,0.000024*0.1};//{P,I,D}
    vector<double> force_pid_gain{0.0001*2,0.0,0.000024*2};//{P,I,D}
...
pos_force_controller::pos_force_controller():nh(), tfBuffer_(), tfListener_(tfBuffer_)
{
  ...
  timer_ = nh.createTimer(ros::Duration(0.001), [&](const ros::TimerEvent& e) {
